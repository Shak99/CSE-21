...
public class ShopArr {
	...
	private void init(int max) {
		...
		for (int i = 3; i < max; i++) {
			cheese[i] = new Cheese("" + (char)('A' + i));//IDK  HELP!
	...
	private double calcSubTotal() {
		...
		double sum= 0;
		for (int i = 0; i < cheese.length; i++){
			sum = cheese[i].getPrice() * cheese[i].getAmount();
			subTotal += sum;
		}
		// Fill-in Code
	...
	private void itemizedList(){
		...
		double[] amtC = new double[cheese.length];
		for (int i = 0; i < cheese.length; i++){
			if(cheese[i].getAmount() > 0) {
				amtC[i]= cheese[i].getAmount() * cheese[i].getPrice();
				System.out.println(cheese[i].getAmount() + " lbs of " + cheese[i].getName() + " @ " + cheese[i].getPrice() + " = $" + amtC[i]);
			}
		}
		// Fill-in Code
	}
